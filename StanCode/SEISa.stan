functions {
  ...
  real[] SI(real t,
      ...
      real dydt[3];   #params[1] = beta, params[2] = epsilon, params[3] = gamma, params[4] = alpha
      
      dydt[1] = - params[1] * y[1] * y[3] +params[3] * y[3] - params[4]*y[1];
      dydt[2] = params[1] * y[1] * y[3]- params[2] * y[2];
      dydt[3] = params[2] * y[2] - params[3] * y[3] + params[4]*y[1];
...
parameters {
  ...
  real<lower = 0, upper = 1> E0; // Initial fraction of exposed 
}

transformed parameters{
  real y_hat[n_obs, n_difeq]; // Output from the ODE solver
  real y0[n_difeq]; // Initial conditions for both S, E, and I
...
model {
  params ~ normal(0, 0.5); //constrained to be positive. this sd was 2
  S0 ~ normal(0.5, 0.5); //constrained to be 0-1.
  E0 ~ normal(0.5, 0.5); //constrained to be 0-1.
