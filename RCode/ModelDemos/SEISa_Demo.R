...
S0 = 1 - (I0+E0) # initial fraction available to adopt


# Assign rates of recruitment, spread, drop-out, and independent uptake:
#lambda = 0.10 #rate of recruitment
beta = 0.45 #rate of spread 0.4
epsilon = 0.60 # rate of progression to adoption
...
inits <- c(S0,E0,I0) #0 denotes that it is an initial condition
...
SEISa <- function(t, y, params) {
  with(as.list(c(params, y)), {
    
    dS =  - beta * y[1] * y[3] +gamma * y[3] - alpha*y[1] #y[1] non-exposed (S) and y[2] is exposed (E) y[3] is adopted (I)
    
    dE = beta * y[1] * y[3]- epsilon * y[2]
    
    dI = epsilon * y[2] - gamma * y[3] + alpha*y[1]
...
# Run the integration:
out <- ode(inits, times, SEISa, params, method="rk")
#
