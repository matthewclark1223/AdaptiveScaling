library(deSolve)
E0 = 0.05    # initial fraction exposed, seed
I0 = 0.02    #initial fraction adopted
S0 = 1 - (I0+E0+M0) # initial fraction available to adopt
M0 = 0.45 #proportion immune 


# Assign rates of recruitment, spread, drop-out, and independent uptake:
#lambda = 0.10 #rate of recruitment
beta = 0.85 #rate of spread 0.4
epsilon = 0.30 # rate of progression to adoption
alpha = 0.02 #independent rate of adoption 0.02
...
inits <- c(S0,E0,I0,M0) #0 denotes that it is an initial condition
...
SEIa <- function(t, y, params) {
  with(as.list(c(params, y)), {
    
    dS =  - beta * y[1] * y[3]  - alpha*y[1] #y[1] non-exposed (S) and y[2] is exposed (E) y[3] is adopted (I)
    
    dE = beta * y[1] * y[3]- epsilon * y[2]
    
    dI = epsilon * y[2] + alpha*y[1]
...
# Run the integration:
out <- ode(inits, times, SEIa, params, method="rk")
#
# Store the output in a data frame:
out <- data.frame(out)
colnames(out) <- c("time", "S","E", "I")
...
lines(out$E ~ out$time, col="green")
#legend(x = 30, y = 0.8, legend = c("Non-adopted", "Adopted"), 
#      col = c("black", "red"), lty = c(1, 1), bty="n")



sample_days = 20 # number of days sampled before fitting
...
sample_propinf = out[out$time %in% sample_time, 4] # 4 here is the 4th (infected) column
