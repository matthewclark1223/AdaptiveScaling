library(tidyverse)
df<-read.csv("./Data/Cleaned/CoasPAs.csv")


sample_n = 205 # total population estimate from coastal PAs data
df<-df%>%filter(Year>=1872)%>%mutate(X=row_number())
...
ggplot(draws, aes(x=as.integer(mod_time), y=value)) +
  ...
  tidybayes::stat_lineribbon(data=filter(draws,as.integer(mod_time)>sample_days),
                             ...
                             color=alpha("gray",0.01),linetype="blank",
                             alpha=0.75) +
  geom_line(data=df,aes(x=X,y=Adopters/sample_n),color="red",linewidth=1.0)+
  geom_vline(xintercept = sample_days+1,linetype="longdash")+
  annotate("text", x = 45, y = .1, label = "True rate",color="red")+
  annotate("text", x = 95, y = .70, label = "Prediction (MAE: 5.3%)",color="#252525")+
