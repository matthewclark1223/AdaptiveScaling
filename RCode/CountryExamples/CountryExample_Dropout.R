library(tidyverse)
df<-read.csv("./Data/Cleaned/Dropout_Included/Chile_TURFS.csv")

df<-df%>%mutate(Cumm_Adopters=Adopters,Adopters=Adopters-Dropout)
sample_n = 1131 # total population estimate from Chile data
...
stan_d = list(n_obs = sample_days,
              n_params = 3, #4 if using SEIS
              n_difeq = 3,  #3 if using SEIS
              n_sample = sample_n,
...
p1<-ggplot(draws, aes(x=as.integer(mod_time), y=value)) +
