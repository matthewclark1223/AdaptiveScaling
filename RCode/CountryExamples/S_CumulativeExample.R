library(tidyverse)
df<-read.csv("./Data/Cleaned/Phili.csv")

#for Philipines only!
df<-df%>%filter(Year>=1978)%>%mutate(X=row_number())
sample_n = 869 # total population estimate from Philippines data
...
ggplot(draws, aes(x=as.integer(mod_time), y=value)) +
  ...
  annotate("text", x = 14, y = .08, label = "True rate",color="red")+
 annotate("text", x = 26, y = .55, label = "Prediction (MAE: 2.4%)",color="#252525")+
